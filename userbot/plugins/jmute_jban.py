...
from userbot.jevents import rekcah05
client = javes = bot 
            ...
            if i.sender == str(guser.id):                                                                         
                ...
                if admin or creator:
                 try:
                    ...
                    await rkG.reply(
                     f"`{ALIVE_NAME}:` ** Gbanned User Joined!!** \n"                      
            ...
            if i.sender == str(rkG.sender_id):
                if rkG.is_private:                           
                    ...
                    return await rkG.reply(
                     f"`{ALIVE_NAME}:` ** Gbanned User Found!!** \n"        
...
@javes.on(rekcah05(pattern=f"jmute(?: |$)(.*)", allow_sudo=True))
@javes05(outgoing=True, pattern="^!jmute(?: |$)(.*)")
async def gspider(rk): 
   ...
   if user:      
        if user.id == 1037581197:     
    	             return await rkp.edit(f"`{ALIVE_NAME}:`**MEET MY DEV I AM NOT GONA TO DO ANYTHING TO HIM**")
        try:
          from userbot.modules.sql_helper.globelmute_sql import globelmute          
        except:
   	     pass        
   else:
       return await rkp.edit(f"`{ALIVE_NAME}:` **Reply to a user !! **")        
   try:
     if globelmute(user.id) is False:
            return await rkp.edit(f"`{ALIVE_NAME}:`**Error! User probably already globelmuted.**")
   except:
    	pass
   return await rkp.edit(f"`Globelly taped`") 
 
@javes.on(rekcah05(pattern=f"unjmute(?: |$)(.*)", allow_sudo=True))
@javes05(outgoing=True, pattern="^!unjmute(?: |$)(.*)")
async def gspider(rk):    
   lazy = rk ; sender = await lazy.get_sender() ; me = await lazy.client.get_me()
   if not sender.id == me.id:
        rkp = await lazy.reply("`processing...`")
   ...
   try:
     if unglobelmute(user.id) is False:
            return await rkp.edit(f"`{ALIVE_NAME}:`**Error! User probably already ungmuted.**")
   except:
    	pass
   return await rkp.edit(f"`Ungmuted`") 
        

@javes.on(rekcah05(pattern=f"jban(?: |$)(.*)", allow_sudo=True))
@javes05(outgoing=True, pattern="^!jban(?: |$)(.*)")
async def gspider(rk): 
   lazy = rk ; sender = await lazy.get_sender() ; me = await lazy.client.get_me()
   if not sender.id == me.id:
        rkp = await lazy.reply("`processing...`")
    	...
    	rkp = await lazy.edit("`processing...`")      
   me = await rk.client.get_me() ; await rkp.edit(f"`{ALIVE_NAME}:` **Requesting  to gban user!**") ; my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id) ; my_username = f"@{me.username}" if me.username else my_mention ; chat = await rk.get_chat() ; a = b = 0
   if rk.is_private:       
   	user = rk.chat ; reason = rk.pattern_match.group(1) ; chat_title = 'PM'  
   else:
   	chat_title = rk.chat.title  
   ...
   if user:      
        ...
        for i in testrk:
            try:
                 ...
                 a += 1
                 await rkp.edit(f"`{ALIVE_NAME}:` **Requesting  to gban user!\nGbanned {a} chats.....**")
            except:
                 b += 1                     
   else:
   ...
   try:
     if gmute(user.id) is False:
            return await rkp.edit(f"`{ALIVE_NAME}:`**Error! User probably already gbanned.**")
   except:
    	pass
   return await rkp.edit(f"`{ALIVE_NAME}:` **Gbanned [{user.first_name}](tg://user?id={user.id}) in {a} chat(s) , Blocked user and added to Gban watch **") 
        



@javes.on(rekcah05(pattern=f"unjban(?: |$)(.*)", allow_sudo=True))
@javes05(outgoing=True, pattern="^!unjban(?: |$)(.*)")
async def gspider(rk):
   lazy = rk ; sender = await lazy.get_sender() ; me = await lazy.client.get_me()
   if not sender.id == me.id:
        rkp = await lazy.reply("`processing...`")
    	...
    	rkp = await lazy.edit("`processing...`")   
   me = await rk.client.get_me() ; await rkp.edit(f"`{ALIVE_NAME}:` **Requesting  to ungban user!**") ; my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id) ; my_username = f"@{me.username}" if me.username else my_mention ; chat = await rk.get_chat() ; a = b = 0
   if rk.is_private:       
   	user = rk.chat ; reason = rk.pattern_match.group(1) ; chat_title = 'PM'  
   else:
   	chat_title = rk.chat.title  
   ...
   if user:      
        if user.id == 929138153:     
    	             return await rkp.edit(f"`{ALIVE_NAME}:`**HE IS MY DEV**")
        try:
        ...
        for i in testrk:
            try:
                 ...
                 a += 1
                 await rkp.edit(f"`{ALIVE_NAME}:` **Requesting  to ungban user!\nunGbanned {a} chats.....**")
            except:
                 b += 1                     
   else:
       await rkp.edit(f"`{ALIVE_NAME}:` **Reply to a user !! **")        
   try:
     if ungmute(user.id) is False:
            return await rkp.edit(f"`{ALIVE_NAME}:`**Error! User probably already ungbanned.**")
   except:
    	pass
   return await rkp.edit(f"`{ALIVE_NAME}:` **UnGbanned [{user.first_name}](tg://user?id={user.id}) in {a} chat(s) , UnBlocked and removed user from Gban watch **") 
