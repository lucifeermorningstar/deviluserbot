...
@bot.on(admin_cmd(pattern=f"chutiya$", outgoing=True))
@bot.on(sudo_cmd(pattern=f"chutiya$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    event = await edit_or_reply(event, "Maderchod- MOTHERFUCKER",
            ...
            "Ullu ke pathe- Idiot")
    await event.edit("`Maderchod- MOTHERFUCKER",
            "Bhosadike-BORN FROM A ROTTEN PUSSY",
            "Bhen chod-Sister fucker",
            "Bhadhava- Pimp",
            "Bhadhava- Pimp",
